...
<?php
// Get all customers
$data = mysqli_query($conn, "SELECT * FROM user WHERE rights = 'U'");

// Build total spent per user from bookings with status = 'Accepted'
$spent = [];
$res = mysqli_query($conn, "
  SELECT uid, SUM(amtpaid) AS total 
  FROM booking 
  WHERE status = 'Completed' 
  GROUP BY uid
");
while ($row = mysqli_fetch_assoc($res)) {
  $spent[$row['uid']] = $row['total'] ? round($row['total']) : 0;
}
...
<head>
  ...
  <link href="aoc/css/bootstrap.min.css" rel="stylesheet">
  <link href="aoc/css/style.css" rel="stylesheet">
  <style>
    body {
    ...
    .table-section {
      ...
      padding: 30px;
      border-radius: 10px;
      box-shadow: 0 0 12px rgba(4, 42, 255, 1);
      color: #ffffff;
    }
    .table th {
...
/* Optional: hover effect for table header cells */
...
<!-- Page Heading -->
<div class="container">
  <h1 class="main-heading"><span class="admin">Admin</span> <span class="dashboard">Our Customers</span></h1>
</div>

<!-- Customer Table -->
<div class="container py-5">
  <h2 class="section-title">Registered Customer Details</h2>
  ...
  $uid = $row['user_id']; // matches booking.uid
  $total = isset($spent[$uid]) ? $spent[$uid] : 0;
?>
<tr>
  <td><?php echo $uid; ?></td>
...
<!-- Scripts -->
<script src="aoc/js/bootstrap.bundle.min.js"></script>
<script src="aoc/js/main.js"></script>
</body>
</html>
